...
function buscarTarefa($mysqli, $id) {
    $sql = "
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("
        UPDATE tarefas 
        SET descricao=?, setor=?, prioridade=?, data_cadastro=?, status_tarefa=?, usuario_responsavel=? 
        WHERE id=?
    ");
...
<div class="container mt-4">
    ...
    <form method="post" class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <input type="text" 
                name="descricao" 
                value="<?= htmlspecialchars($tarefa['descricao']) ?>" 
                class="form-control" 
                required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <input type="text" 
                name="setor" 
                value="<?= htmlspecialchars($tarefa['setor']) ?>" 
                class="form-control" 
                required>
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="usuario_responsavel" class="form-select" required>
                <?php while ($u = $usuarios->fetch_assoc()): ?>
                    <option value="<?= $u['id'] ?>" 
                        <?= $u['id']==$tarefa['usuario_responsavel'] ? 'selected' : '' ?>>
        ...
        <div class="col-12">
            ...
            <a href="read_gerenciamento.php" class="btn btn-secondary">Cancelar</a>
        </div>
